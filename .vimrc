...
"set gfn=CMU\ Typewriter\ Text\ Bold\ 14 "set gfn=Ubuntu\ Mono\ 13
...
nnoremap <F3> :call VimDiff()<CR>
autocmd filetype cpp nnoremap <S-F5> :lcd %:p:h <bar> !clear && clear && g++ -std=gnu++20 -Wall -Wextra -Wshadow -Wfloat-equal -Wconversion -Wshift-overflow=2 -Wduplicated-cond -O2 -DCONVICTION %
"autocmd FileType cpp nnoremap <F5> :lcd %:p:h <bar> !~/compile.sh %<CR>
autocmd filetype cpp nnoremap <F5> :lcd %:p:h <bar> !clear && clear && ~/Dropbox/VimConfig/Scripts/compile.sh %
...
command! -range YankToSubmit call YankToSubmit()
...
nnoremap <F8> :call CopyFileContent()<CR>
nnoremap <F4> :call Erase()<CR>
nnoremap <F10> :source ~/Dropbox/VimConfig/.vimrc
"<F11> for toggling toolbar and menubar
autocmd filetype c,cpp nnoremap <F12> :call Toerso()<CR>
autocmd filetype c,cpp nnoremap <S-F12> :call ToersoAddress()<CR>
...
nnoremap <leader>cd :Explore ~/Dropbox/Share<CR>
...
nnoremap <leader>my :silent exec 'terminal w3m https://codeforces.com/submissions/Toerso'<CR>
...
function! GetCodeforcesProblemIdFromComment()
    " Search for the pattern 'Problem ID: <alphanumeric code>'
    let pattern = '\vProblem ID: \zs(\S+)'
    let line_with_tag = search(pattern, 'nw')

    if line_with_tag != 0
        let problem_id = matchstr(getline(line_with_tag), pattern)
        return problem_id
    else
        return ''
    endif
...
" Function to yank the selected range and open Codeforces submission page
function! YankToSubmit() range
    let problem_id = GetCodeforcesProblemIdFromComment()

    if problem_id == ''
        echom 'Failed to extract Codeforces Problem ID from comment'
        return
    endif

    let pattern = '\v(\d+)(\D)'
    let match = matchlist(problem_id, pattern)

    let submission_url = 'https://codeforces.com/problemset/submit/' . match[1].'/'.match[2]
    call system('xdg-open ' . submission_url . ' &')
endfunction

function! Toerso()
    execute "normal! ggVG\"+y"
    exec 'YankToSubmit'
    "exec '1,8d | w'  " Uncomment this line if you want to delete lines 1 to 8 after yanking
endfunction

function! ToersoAddress()
   exec 'YankToSubmit'
   let @+=expand("%:p")
   echo expand("%:p")
...
function! Submit()
   exec '0r ~/Dropbox/Space/Practice/practice.cpp'
   write
